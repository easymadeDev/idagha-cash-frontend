...
export default function AdminMembers() {
  ...
  const toggleWaSubscribed = async (m: any) => {
    const endpoint = m.whatsappSubscribed ? 'unsubscribe' : 'subscribe';
    try {
      await api.post(`/whatsapp/${endpoint}/${m._id}`);
      mutate('/api/members/admin/all');
    } catch {
      toast('Failed to update WhatsApp status.', 'error');
    }
  };
                  ...
                  <div className="admin-member-card-top">
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 3 }}>
                        ...
                        {m.whatsappSubscribed
                          ? <span title="WhatsApp subscribed" style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: '0.6rem', padding: '1px 6px', borderRadius: 99, background: 'rgba(37,211,102,0.1)', border: '1px solid rgba(37,211,102,0.3)', color: '#25d366', fontWeight: 700 }}>
                          ...
                          : <span title="Not subscribed to WhatsApp" style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: '0.6rem', padding: '1px 6px', borderRadius: 99, background: 'rgba(107,114,128,0.08)', border: '1px solid var(--border)', color: 'var(--text-3)', fontWeight: 600 }}>
                              WA ✗
                            </span>}
                      </div>
                    </div>
